...
export function bunga(imageData, canvas, koordinat, rad, n, warna){
    let [xc, yc] = koordinat;
...
export function bunga_n(imageData, canvas, koordinat, rad, rad_bunga, n, warna){
    let [xc, yc] = koordinat;
    var temp = (Math.PI * 2) / n;

    for (var theta = 0; theta < n; theta ++){
        let x = xc + rad * Math.cos((theta * temp) + (Math.PI / 4));
        let y = yc + rad * Math.sin((theta * temp) + (Math.PI / 4));
        bunga(imageData, canvas, [x, y], rad_bunga, n, warna);
    }
}

export function fill_n(imageData, canvas, koordinat, rad, n, toFlood, warna){
    let [xc, yc] = koordinat;
    var temp = (Math.PI * 2) / n;

    for (var theta = 0; theta < n; theta ++){
        let x = xc + rad * Math.cos((theta * temp));
        let y = yc + rad * Math.sin((theta * temp));
        floodFillStack(imageData, canvas, Math.ceil(x), Math.ceil(y), toFlood, warna);
    }
}

export function setengah_lingkaran(imageData, canvas, koordinat, rad, warna){
    let [xc, yc] = koordinat;

    for (var theta = 0; theta <= Math.PI; theta += 0.001){    
        let x = xc + rad * Math.cos(theta);
        let y = yc + rad * Math.sin(theta);
...
export function elips_polar(imageData, canvas, koordinat, rad, warna){
    let [xc, yc] = koordinat;
    let [radX, radY] = rad;

    for (var theta = 0; theta < Math.PI*2; theta += 0.001){
        let x = xc + radX * Math.cos(theta);
        let y = yc + radY * Math.sin(theta);
